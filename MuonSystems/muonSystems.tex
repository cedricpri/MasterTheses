...
\item Calculate the likelihood of a given measurement;
...
\begin{itemize}
\item First of all, a muon is defined from the \textbf{MuonState} class and from 7 parameters: 3 parameters ($x$, $y$, $z$) representing the position of the momentum vector of a muon, 3 additional parameters ($v_x$, $v_y$, $v_z$) representing its three-dimensional direction and the actual value of its momentum. The measurement performed at the top detector will be labelled with the index 1 ($x_1$, $y_1$, $z_1$, $v_{x1}$, $v_{y1}$, $v_{z1}$, $p_1$) while the measurement performed by the lower detector will take the index 2.
\item \textbf{Surfaces} and \textbf{Volumes}, virtual classes allowing us to define general surfaces (their spatial position and their geometrical center) and volumes, defined as vectors of several surfaces: a general volume is in this sense defined by its position and global density. Defining such virtual classes is extremely important since it would allow us to consider different geometries, not just a pipe put in between the two detectors. The constant $z$ position of both the lower and upper detectors is also defined at this stage.
\item \textbf{Cylinders} and \textbf{Pipes}, particular subclasses of the virtual classes previously defined in order to define the geometry of this particular problem. A pipe is in this sense defined as a set of two cylinders and from 7 different parameters: its central position ($x$, $y$, $z$), along with its density, the radii $r$ and $R$ of both cylinders and its length along the y-axis $L$, as shown in Figure~\ref{fig:cylinder}.
...
From this, computing the interesection between such cylinder and a general MuonState ($x_0$, $y_0$, $z_0$, $v_{x}$, $v_{y}$, $v_{z}$, $p$) is trivial, since such MuonState can be represented by a straight line: 
...
By putting this into the euqation of the cylinder, we get the following expression:
...
Once both the squared values applied, we get an equation having the shape $Ax^2 + Bx + C = 0$, where both cutting points will be given by $x^{+/-} = \frac{-B \pm \sqrt{B^2 - 4 AC}}{2A}$:
...
\begin{itemize}
\item First of all, the distance between the MuonState and all the surfaces of the object considered is computed and the first intersection point is kept.
...
\item This propagation process is then repeated 6 times in total, until reaching a position extremely close to the volume that is being studied.
\item This whole proces is repeated for all the different surfaces encountered by the muon (up to four times in total), and one final time until reaching the bottom detector. At this point, the MuonState is kept as the second measurement ($x_2$, $y_2$, $z_2$, $v_{x2}$, $v_{y2}$, $v_{z2}$, $p_2$).
\end{itemize}

\item The \textbf{Likelihood} object can then be calculated. This class reads a Monte-Carlo generated file (explanations regarding this process will be given in Section~\ref{sec:Generator}) and, for each generated event, computes the $\Delta x$, $\Delta \theta_x$, $\Delta y$ and $\Delta \theta_y$ parameters defined between the outgoing muon and the prolongation of the incoming muon vector and representing the actual deviation applied to the muon, as shown in Figure~\ref{fig:parameters}.

\begin{figure}[htbp]
\centering
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=6cm]{figs/parameters.png}
\end{minipage}\hfill
\caption{Typical example of two incident muons not respecting the normal incidence required by the Moliere equation for the multiple scattering of muons.}
\label{fig:parameters}
\end{figure}

This process is repeated over and over again (1000 times in total), since we known that the multiple scattering is a stochastic process. Each time the process is executed, different deviations values are therefore obtained, each kept in a vector. A p-value for the actual measurement is then computed by using the vectors previously defined and representing an histogram of all the possible deviation values. The p-values computed along the x and y-axis then enter the global likelihood value that is used for the minimization process.

\item \color{red} Talk about the minimization process once implemented. \color{black}
\end{itemize}

\section{Generator} \label{sec:Generator}

As we have just seen, the computation of the p-values required for the likelihood takes as input a file of Monte-Carlo generated events, which has been previously produced thanks to this \textbf{Generator}. The Generator is in this sense able to take as input one rootfile containing the Monte-Carlo generation of incoming cosmic muons and its main goal is then to read the input parameters of such muons and to propagate using the previously defined functions, to generate a dsitribution of the expected output.

\color{red} Talk about MC truth and measurement. \color{black}

\chapter{Results obtained}

\section{Incident muons distribution}
